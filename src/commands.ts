...
function buildScanPrompt(
  ...
  if (!isChinese(language)) {
    return [
      ...
      '',
      'Requirements: every finding needs a file locator and persona_refs; describe what users observe, not missing code. Use Nielsen’s ten heuristics as the base checklist, then apply the product-specific focus. Respect each rule’s minimum evidence level; do not turn source candidates into visual or interaction conclusions.',
  ...
  return [
    ...
    '- 以 Nielsen 十项可用性原则为基础清单，再叠加产品类型重点；',
    '- 严格遵守每条规则的最低证据等级，不把源码候选直接写成视觉或交互结论；',
    '- 报告只做"提醒开发者去看一眼"，不改代码。',
    '',
